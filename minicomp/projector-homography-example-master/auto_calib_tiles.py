...
def draw_tiles(image, tilevalues, nrows, ncols,howlong):
	pos = 0
	tilesize = 512 #512 = 1024/2 = 1024/nrows
	hm = abs((howlong % 10) - 5)
	for row in range(nrows):
		for col in range(ncols):
			...
			y0 = row * tilesize
			y1 = y0 + tilesize
			color = int(value * 255.0)#(51*hm))#(255.0)
...
if __name__ == '__main__':
    ...
    nrows = 2#NOTE: if you change this, make corresponding change in draw_tiles()
    ncols = 2#NOTE: if you change this, make corresponding change in draw_tiles()
    ...
    k = -1
    ser=serial.Serial(port=7, baudrate=9600,timeout=0.2) #port = n-1 if Arduino is in COM(n)
    howlong = 0; 
